...
This script avoids third-party metric libraries so it can run in the current
project environment.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate model predictions from a CSV file."
    )
    parser.add_argument("--csv-path", required=True, help="Path to predictions CSV.")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Threshold used when pred column is absent.",
    )
    parser.add_argument(
        "--top-fraction",
        type=float,
